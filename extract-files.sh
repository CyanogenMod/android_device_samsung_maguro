...
for x in `cat proprietary-blobs.txt |grep ^/system`
do
	BIN=`echo $x |awk -F/ '{print $NF}'`
	PROP_PATH="../../../vendor/$MANUFACTURER/$DEVICE/proprietary/$BIN"
	echo "Putting $BIN: $PROP_PATH"
	adb pull $x $PROP_PATH 2> /dev/null
done
